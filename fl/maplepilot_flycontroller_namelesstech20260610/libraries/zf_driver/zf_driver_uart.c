...
#define UART_USE_ISR    CPUIntIdx6_IRQn
...
void uart_init_intidx(uart_index_enum uart_n, uint32 baud, uart_tx_pin_enum tx_pin, uart_rx_pin_enum rx_pin,IRQn_Type int_idx,uint32_t PreemptPriority, uint32_t SubPriority)
{
    ...
    cy_stc_sysint_irq_t                 uart_irq_cfg;
    uart_irq_cfg.sysIntSrc = uart_pin_config.uart_irqn;
    uart_irq_cfg.intIdx    = int_idx;
    uart_irq_cfg.isEnabled = true;
    //interrupt_init(&uart_irq_cfg, uart_isr_func[uart_n], 0);
    interrupt_init_EncodePriority(&uart_irq_cfg, uart_isr_func[uart_n], PreemptPriority,SubPriority);
}
