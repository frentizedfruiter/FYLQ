...
void nadc_init(void)
{
  adc_init(ADC_KEY1_ADC_PIN, ADC_12BIT);
  adc_init(ADC_KEY2_ADC_PIN, ADC_12BIT);
  adc_init(BATTERY_VOLTAGE_ADC_PIN, ADC_12BIT);
}
...
{
  adc_value[0]=adc_convert(ADC_KEY1_ADC_PIN); 
  adc_value[1]=adc_convert(ADC_KEY2_ADC_PIN); 
  adc_value[2]=adc_convert(BATTERY_VOLTAGE_ADC_PIN); 
  float bat=(DIVIDER_RES_SCALE*adc_value[2]*3.3f)/4096;
  return bat;
}
